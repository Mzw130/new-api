  ...
  persist(
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state, error) => {
        if (error || !state?.config) return
        const logo = normalizeSystemLogo(state.config.logo)
        const loadedLogoUrl = normalizeSystemLogo(state.loadedLogoUrl)
        if (logo !== state.config.logo || loadedLogoUrl !== state.loadedLogoUrl) {
          useSystemConfigStore.setState((s) => ({
            config: { ...s.config, logo },
            loadedLogoUrl,
          }))
        }
      },
