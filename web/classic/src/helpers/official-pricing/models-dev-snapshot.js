...
import { fetchModelsDevOfficialPrices } from './models-dev';
...
export async function fetchOfficialPriceChanges() {
  const current = await fetchModelsDevOfficialPrices();
  const prev = loadSnapshot();
  return detectChanges(current, prev);
}

export async function acknowledgeOfficialPriceSnapshot() {
  const current = await fetchModelsDevOfficialPrices();
